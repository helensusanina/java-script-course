...
const paymentForm = el("form#paymentForm", [
    ...
    el("input#expirationDate", {
        type: "text",
        placeholder: "Действительна до (ММ/ГГ)",
        required: true,
    }),
...
function setCardLogo(cardType) {
    ...
    if (cardType && cardLogos[cardType]) {
        ...
        const elem = document.getElementById("cardLogoContainer");
        elem.appendChild(cardLogo);
...
function checkFormValidity(qualifiedName, value) {
    ...
    const validateFields = {
        ...
        expirationDate:
            expirationDate.length === 4 && +expirationDate.slice(0, 2) <= 12,
        cvc: validator.cvv(cvc).isValid,
        email: emailRegex.test(email),
    };
    payButton.disabled = Object.values(validateFields).includes(false);
}
[cardNumberInput, expirationDateInput, cvcInput, emailInput].forEach(
    (input) => {
